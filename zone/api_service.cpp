...
Json::Value ApiSetSpawnLocation(EQ::Net::WebsocketServerConnection* connection, Json::Value params) {
	...
	if (!params.isArray() || params.size() != 4 || !params[0].isInt() || !params[1].isNumeric() || !params[2].isNumeric() || !params[3].isNumeric()) {
		...
		response["error"] = "Parameters incorrect, Usage: [spawn_id, X, Y, Z]";
		return response;
	}
	...
	if (s2 == nullptr) {
		response["status"] = "ERROR";
		response["error"] = "Cannot find spawn_id provided";
		return response;
	}

	std::string query = StringFormat("UPDATE spawn2 SET x = '%f', y = '%f', z = '%f', heading = '%f' WHERE id = '%i'",
		params[1].asFloat(), params[2].asFloat(), params[3].asFloat(), s2->GetHeading(), s2->GetID());
	...
	auto npc_id = s2->SpawnedNPCID();
	bool repop = false;
	auto* npc = entity_list.GetNPCByID(npc_id);
	if (npc != nullptr) {
		npc->Depop();
		repop = true;
	}
	...
	delete s2;
	s2 = database.LoadSpawn2(zone->spawn2_list, id, 0);
	if (repop) {
		s2->Repop();
	}
	...
	entity_list.SetWSApiHandler(
		[&](const EQApplicationPacket* app) {
			switch (app->GetOpcode()) {
				...
				case OP_MoveDoor: { // 339
					auto md = (MoveDoor_Struct*)app->pBuffer;
					Json::Value data;
					data["id"] = md->doorid;
					data["door_action"] = md->action;
					data["action"] = "MoveDoor";
					server->DispatchEvent(EQ::Net::SubscriptionEventClientUpdate, data, 50);
					break;
				}
				case OP_WearChange: { // 536
					auto wc = (WearChange_Struct*)app->pBuffer;
					Json::Value data;
					data["id"] = wc->spawn_id;
					data["wear_slot"] = wc->wear_slot_id;
					data["material"] = wc->material;
					data["color"] = wc->color.Color;
					data["action"] = "WearChange";
					server->DispatchEvent(EQ::Net::SubscriptionEventClientUpdate, data, 50);
					break;
				}
				case OP_PlayerStateAdd: { // 376
					auto ps = (PlayerState_Struct*)app->pBuffer;
					Json::Value data;
					data["id"] = ps->spawn_id;
					data["state"] = ps->state;
					data["action"] = "PlayerStateAdd";
					server->DispatchEvent(EQ::Net::SubscriptionEventClientUpdate, data, 50);
					break;
				}
				case OP_SpawnAppearance: { // 477
					auto sa = (SpawnAppearance_Struct*)app->pBuffer;
					Json::Value data;
					data["id"] = sa->spawn_id;
					data["parameter"] = sa->parameter;
					data["type"] = sa->type;
					data["action"] = "SpawnAppearance";
					server->DispatchEvent(EQ::Net::SubscriptionEventClientUpdate, data, 50);
					break;
				}
				case OP_TimeOfDay: { // 501
					auto td = (TimeOfDay_Struct*)app->pBuffer;
					Json::Value data;
					data["year"] = td->year;
					data["month"] = td->month;
					data["day"] = td->day;
					data["hour"] = td->hour;
					data["minute"] = td->minute;
					data["action"] = "TimeOfDay";
					server->DispatchEvent(EQ::Net::SubscriptionEventClientUpdate, data, 50);
					break;
				}
				default: {
					std::cout << "Sent unknown server->client packet with OpCode " << app->GetOpcode() << std::endl;
					break;
				}
			}
		}
